...
Another key contribution is to support dynamic reconfiguration,
allowing the system to adapt to the changes in the infrastructure and external conditions.
%
...
%
To do so,
the project will leverage AI and machine learning techniques to manage the complex task of reconfiguration
to opportunistically balance the performance, energy consumption or any other system-specific \ac{qos}.

\paragraph{Bind pulverization with other CAS frameworks}
The pulverization approach is designed to be independent of a specific technology or framework.
%
Nevertheless,
as a consequence of the point described above,
to be effectively used in a real-world scenario,
the pulverization approach must be integrated with other \ac{cas} frameworks.
%
ScaFi~\cite{DBLP:journals/softx/CasadeiVAP22} and Protelis~\cite{DBLP:conf/sac/PianiniVB15} are two examples of consolidated frameworks for engineering \ac{cas}.
%
In particular,
they are both based on the \ac{ac} paradigm,
which is a possible way to implement \ac{cas}.
%
Integrating the pulverization framework with these two frameworks,
allows exploiting the pulverization approach to design and deploy \ac{cas} on heterogeneous infrastructures.

\paragraph{Data stream management}
Due to the high volume of data produced by \ac{cas},
...
%
Concluding, we will try to provide a way to access the data produced by the system efficiently and effectively,
enabling data analysis techniques to be applied to the data.
