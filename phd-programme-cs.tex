...
like \ac{iot}, swarm robotics, smart cities, and humans enhanced by wearable devices~\cite{DBLP:conf/aisi/JumaS19, DBLP:journals/sensors/LosetoSRGIFBL22}.
...
where offloading some of the computation to the cloud is a common practice.
%
However,
...
making their \emph{opportunistic exploitation} a challenging task~\cite{DBLP:journals/scpe/Petcu21}. 
%
...
but also infrastructure nodes (i.e. cloud server, mobile device, wearable device).
%
...
%
In this regard,
aspects like \emph{interoperability, standardization, portability, and open source principles} are fundamental to achieving openness~\cite{DBLP:journals/computing/SteenT16}.
%
However,
the hard part is represented by the semantic specification of the interface,
which usually,
are provided in a natural language,
and not in a formal and machine-readable way.
%
...
making it difficult to provision and setup the infrastructure~\cite{DBLP:conf/synasc/VladusicR20}.
%
...
but also the interoperability between the different devices~\cite{DBLP:journals/midm/TorabMiandoabSJR23}.
%
Ideally,
...
with the help of \ac{ai} techniques~\cite{DBLP:journals/computing/ShahidaniGHK23}.
